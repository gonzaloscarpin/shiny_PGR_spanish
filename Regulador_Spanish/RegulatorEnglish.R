...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            numericInput("actual_height", "Actual plant Height (cm):", value = 93.3, min = 0),
            numericInput("num_nodes", "Actual number of Nodes:", value = 14, min = 1),
            numericInput("row_distance", "Row Distance (cm):", value = 91, min = 0),
            numericInput("n_plants", "N of plants per m:", value = 6, min = 0),
            numericInput("branch_node", "Node of the 1st Fruiting branch:", value = 6, min = 0),
            numericInput("product_concentration", "PGR product concentration (%):", value = 5, min = 0),
            numericInput("product_applied", "PGR already applied (ml/ha):", value = 0, min = 0),
            actionButton("calculate", "Calculate", class = "btn-primary")
        ),
        mainPanel(
            tabsetPanel(
                tabPanel("Result:", verbatimTextOutput("calculations")),
                tabPanel("Plot:", plotOutput("heightPlot"))
            )
        )
    )
)
...
server <- function(input, output) {
    observeEvent(input$calculate, {
        ...
        output$calculations <- renderPrint({
            cat("Adjusted PGR Need (ml/ha):", final_recommended_PGR_rate_show, "\n")
        })
        ...
        output$heightPlot <- renderPlot({
            ...
            ggplot(df, aes(x = Nodes)) +
                geom_line(aes(y = Relation_NN_Height, color = "Theoretical Height")) +
                geom_point(aes(x = input$num_nodes, y = input$actual_height, color = "Actual Height"), size = 3) +
                scale_x_continuous(breaks = 0:26) +
                scale_y_continuous(breaks = seq(0, input$final_height, by = 10)) +
                labs(x = "Number of Nodes",
                     y = "Cotton Plant Height (cm)",
                     title = "Cotton Plant Height Theoretical Model",
                     color = "Legend"
                ) +
                scale_color_manual(values = c("Theoretical Height" = "blue", "Actual Height" = "red")) +
                theme_minimal() +
